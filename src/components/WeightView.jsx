...
export default function WeightView({ weights, addWeight, deleteWeight }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-6 space-y-6">
      <div>
        <h2 className="text-lg font-bold text-gray-900 mb-1">Weight Tracker</h2>
        <p className="text-sm text-gray-400">106 kg → 90 kg · −16 kg target</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4">
        <div className="flex justify-between text-xs text-gray-400 mb-2">
          ...
          <span className="font-semibold text-gray-600">{progress.toFixed(0)}% complete</span>
      ...
      <div className="grid md:grid-cols-[1fr_320px] gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <p className="text-sm font-semibold text-gray-700 mb-4">Progress chart</p>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={chartData} margin={{ top: 4, right: 16, bottom: 0, left: -10 }}>
              ...
              <YAxis domain={[Math.max(50, GOAL_KG - 5), START_KG + 2]} tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 10, border: '1px solid #e5e7eb' }} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Line type="monotone" dataKey="actual" name="Actual (kg)" stroke="#E24B4A" strokeWidth={2.5} dot={{ r: 3, fill: '#E24B4A', strokeWidth: 0 }} connectNulls={false} />
              <Line type="monotone" dataKey="target" name="Target (kg)" stroke="#22c55e" strokeWidth={2} strokeDasharray="5 5" dot={false} connectNulls />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100">
            <p className="text-sm font-semibold text-gray-700 px-4 pt-4 pb-2">Weight log</p>
            {sorted.length === 0 ? (
